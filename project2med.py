...
def Q_learning(Q, data):
    ...
    while round(curr_mean, 2) != round(prev_mean, 2):  # continue until significant increase in mean of Q-matrix
...
def main():
    ...
    data = read('medium.csv')
    Q_final = Q_learning(Q, data)
    P = np.argmax(Q_final, axis=1) + 1
    default = 4  # default action if Q matrix row is all zeros (avg possible acceleration)
    for i in range(len(Q)):
        all_zeros = not np.any(Q[i])
        if all_zeros:
            P[i] = default
    print(Q_final)
    print("print P")
    ...
    return write_pol(P, "medium.policy")
